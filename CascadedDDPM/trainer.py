...
def find_first(fn, arr):
    '''
    Apply a function iteratively on the elements of an array and see if the function
    succeeds. If not, continue to next element, until reaching index where function does not
    return empty. Then return index. 
    If no element in array gives an ouput, return -1
    ...
    Returns: 
    - ind: index corresponding to first success on fn
    or
    - -1
    '''
...
def pick_and_pop(keys, d):
    '''
    Function to remove specified keys (and values?) from a dictionary.

    Parameters:
    - keys: keys to 'pop' (remove) from dictionary
